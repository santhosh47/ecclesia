"""Attendance and check-in endpoints with automated absentee detection."""
...
def get_absentee_alerts(
    ...
    for member in members:
        # Find latest attendance record where status is Present or Late
        ...
        if last_date is None:
            # Never attended or created recently
...
def get_attendance_summary(db: Session = Depends(get_db)) -> AttendanceSummary:
    ...
    # Average Sunday Attendance (last 4 Sundays)
    sunday_events = list(
        db.scalars(
            select(Event)
            .options(joinedload(Event.attendance_records))
            .where(Event.event_type.ilike("%Sunday%"))
    ...
    if sunday_events:
        total_counts = [
            e.headcount_adults + e.headcount_children + len(e.attendance_records) for e in sunday_events
        ]
